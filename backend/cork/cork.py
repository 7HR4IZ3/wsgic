...
class BaseCork(object):
	...
	def __init__(self, directory=None, backend=None, email_sender=None,
				 ...
				 smtp_url='localhost', environ_key="wsgic_session", fail_redirect=None):
	...
	def login(self, username, password, success_redirect=None,
		...
		if username in self.db.users:
			salted_hash = self.db.users[username]['hash']
			...
			if authenticated:
				...
				self.db.users[username]['last_login'] = str(
					datetime.utcnow())
				self.db.save_users()
				if success_redirect:
		...
		if fail_redirect:
			self._redirect(fail_redirect or self.fail_redirect)

		return False

	def logout(self, success_redirect='/login', fail_redirect=None):
		...
		try:
			...
			self._redirect(fail_redirect or self.fail_redirect or "/")

		self._redirect(success_redirect)

	def login_required(self, username=None, role=None, fixed_role=False,
				fail_redirect=None):
		...
		if role is not None and role not in self.db.roles:
		...
		# Authorization
		if cu.role not in self.db.roles:
		...
		if role is not None:
			...
			current_lvl = self.db.roles[self.current_user.role]
			threshold_lvl = self.db.roles[role]
	...
	def list_roles(self):
		...
		"""
		for role in sorted(self.db.roles):
			yield (role, self.db.roles[role])
	...
	# def register_user(self, username, role, password, email_addr=None,
	# 				description=None):
	# 	"""Create a new user account.
	# 	This method is available to users with level>=100
	...
	def list_users(self):
		...
		:return: (username, role, email_addr, description) generator (sorted by
			username)
		"""
		for un in sorted(self.db.users):
			d = self.db.users[un]
	...
	def current_user(self):
		...
		if username is None:
			raise AuthException("Unauthenticated user")
		if username is not None and username in self.db.users:
	...
	def logged_in(self):
		return not self.user_is_anonymous
	...
	def register(self, username, password, email_addr, role='user',
				 max_level=50, subject="Account Confirmation",
		...
		if role not in self.db.roles:
			raise AAAException("Nonexistent role")
		if self.db.roles[role] > max_level:
		...
		h = self._hash(username, password)
		h = h.decode('ascii')
		self.db.pending_registrations[registration_code] = {
			'username': username,
			'role': role,
			'hash': h,
			'email_addr': email_addr,
		...
		self.db.save_pending_registrations()
	...
	def validate_registration(self, registration_code):
		...
		"""
		try:
			data = self.db.pending_registrations.pop(registration_code)
		except KeyError:
	...
	def send_password_reset_email(self, username=None, email_addr=None,
		...
		**kwargs):
		...
		if not username:
			...
			for k, v in self.db.users.iteritems():
			...
			if not email_addr:
				email_addr = self.db.users[username].get('email_addr', None)
				if not email_addr:
					raise AAAException("Email address not available.")

			else:
				# both username and email_addr are provided: check them
				stored_email_addr = self.db.users[username]['email_addr']
	...
	def require(self, username=None, role=None, fixed_role=False, fail_redirect=None):
		...
		def decorator(func):
			self.login_required(username=username, role=role, fixed_role=fixed_role, fail_redirect=fail_redirect)
			return func
		return decorator
	...
	def anonymous(self, func):
		if self.user_is_anonymous:
			...
			# else:
			self._redirect(self.fail_redirect)
	...
	def setup_demo(self):
	    self.db.roles['admin'] = 100
	    self.db.roles['editor'] = 60
	    self.db.roles['user'] = 50
	    self.db.save_roles()
...
class User(object):
	...
	def __init__(self, username, cork_obj, session=None):
		...
		"""
		self._cork = cork_obj
		assert username in self._cork.db.users, "Unknown user"
		self.username = username
		user_data = self._cork.db.users[username]
		self.role = user_data['role']
		self.description = user_data['desc']
		self.email_addr = user_data['email_addr']
		self.level = self._cork.db.roles[self.role]
	...
	def update(self, role=None, pwd=None, email_addr=None):
		...
		"""
		username = self.username
		if username not in self._cork.db.users:
			raise AAAException("User does not exist.")

		if role is not None:
			if role not in self._cork.db.roles:
				raise AAAException("Nonexistent role.")

			self._cork.db.users[username]['role'] = role

		if pwd is not None:
			self._cork.db.users[username]['hash'] = self._cork._hash(
				username, pwd).decode()

		if email_addr is not None:
			self._cork.db.users[username]['email_addr'] = email_addr

		self._cork.db.save_users()
	...
	def delete(self):
		...
		"""
		try:
			self._cork.db.users.pop(self.username)
		except KeyError:
			raise AAAException("Nonexistent user.")
		self._cork.db.save_users()
