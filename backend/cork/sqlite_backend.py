...
#         cc = ','.join(cc)
#         query = "CREATE TABLE IF NOT EXISTS %s (%s)" % (self._table_name, cc)
...
from wsgic.helpers.database.sqlite import SqliteDatabase
...
class Table(db.Model):
	...
	def make(self):
		...
		for x in self.columns:
			c = str(self.columns[x])
			n, v = c.split(" ")[:2]
			s.append((x, v))
		self._columns = tuple(s)

		self._column_names = [x for x, t in self._columns]
		self._key_col_num = 0
		self._key_col_name = self._column_names[self._key_col_num]
		self._key_col = self._column_names[self._key_col_num]
	...
	def __contains__(self, key):
		#FIXME: count()
		query = "SELECT * FROM %s WHERE %s='%s'" % \
			(self.tableName, self._key_col, key)
	...
	def __setitem__(self, key, value):
		...
		question_marks = ', '.join('?' for x in col_values)
		query = "INSERT OR REPLACE INTO %s (%s) VALUES (%s)" % \
			(self.tableName, col_names, question_marks)

		ret = self.db.execute(query, args=col_values)


	def __getitem__(self, key):
		query = "SELECT * FROM %s WHERE %s='%s'" % \
			(self.tableName, self._key_col, key)
		row = self.db.execute(query, one=True)
		if row is None:
			print(self.db.execute("SELECT * FROM users"))
			raise KeyError(key)
	...
	def __iter__(self):
		"""Iterate over table index key values"""
		query = "SELECT %s FROM %s" % (self._key_col, self.tableName)
	...
	def pop(self, key):
		...
		query = "DELETE FROM %s WHERE %s='%s'" % \
			(self.tableName, self._key_col, key)
		self.db.execute(query, one=True)
		#FIXME: check deletion
		return d
...
class SQLiteBackend:
	def __init__(self, users, roles, pending, initialize=False):
		self.users = users()
		self.roles = roles()
		...
		if initialize:
			self.users.create()
			self.roles.create()
			self.pending_registrations.create()
