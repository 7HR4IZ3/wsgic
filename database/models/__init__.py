...
class Model(Hooks, metaclass=ModelMetaclass):
    ...
    def reload(self):
        ...
        if getattr(self, "id"):
            ...
            kw = {x: self[x] for x in self.asdict() if self[x] is not None}
        data = self.Meta.objects.get(**kw)
        if data:
            self.__update_data__(**data[0].asdict())
        return True
    ...
    def save(self):
        ...
        data.pop("id", None)
        if getattr(self, "id"):
            self.objects.update(data, id=self.id)
            self.reload()
        else:
