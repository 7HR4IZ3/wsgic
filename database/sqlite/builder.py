...
self,      * allow limit update queries with WHERE.
    ...
    def join(self, table, cond, type = '', escape = None):
        ...
        if (not self.hasOperator(cond)):
            ...
            for i in conditions:
                ...
                cond += joints[i]
                match = re.match('/(\(*)?([\[\]\w\.\'-]+)' + preg_quote(operator, '/') + '(.*)/i', condition)
    ...
    def whereHaving(self, qbKey, key, value = None, type = 'AND ', escape = None):
        ...
        for k in key:
            ...
            elif (not self.hasOperator(k) and qbKey != 'QBHaving'):
                # value appears not to have been set, assign the test to IS None
                k += ' IS None'
            elif (preg_match('/\s*(!?=|<>|IS(?:\s+NOT)?)\s*/i', k)):
                match = preg_match('/\s*(!?=|<>|IS(?:\s+NOT)?)\s*/i', k)
    ...
    def _whereIn(self, ?key = None, values = None, not = False, type = 'AND ', escape = None, clause = 'QBWhere'):
        ...
        if (values == None || (not is_array(values) and not (values instanceof Closure))):
            if (CI_DEBUG):
    ...
    def _like(self, field, match = '', type = 'AND ', side = 'both', not = '', escape = None, insensitiveSearch = False, clause = 'QBWhere'):
        ...
        foreach (field as k : v):
            ...
            if (escape == True and self.db.likeEscapeStr != ''):
    ...
    def orderBy(self, orderBy, direction = '', escape = None):
        ...
        if (escape == False):
            ...
            foreach (orderBy.split(',') as field):
                qbOrderBy[] = (direction == '' and preg_match('/\s+(ASC|DESC)/i', rfield.strip(), match, PREG_OFFSET_CAPTURE))
                    ? [
    ...
    def _limit(self, sql, offsetIgnore = False):
        return sql + ' LIMIT ' + (offsetIgnore == False and self.QBOffset ? self.QBOffset + ', ' : '') + self.QBLimit
    ...
    def compileFinalQuery(self, sql):
        ...
        if (not not (self.db.swapPre) and not not (self.db.DBPrefix)):
    ...
    def compileIgnore(self, statement):
        if (self.QBIgnore and isset(self.supportedIgnoreStatements[statement])):
            ...
            foreach (self.{qbKey as &qbkey):
                ...
                foreach (conditions as &condition):
                    if ((op = self.getOperator(condition)) == False
                        || not preg_match('/^(\(?)(.*)(' + preg_quote(op, '/') + ')\s*(.*(?<!\)))?(\)?)/i', condition, matches)
                    ):
                        continue
    ...
    def compileOrderBy(self, ):
        if (is_array(self.QBOrderBy) and not not (self.QBOrderBy)):
            foreach (self.QBOrderBy as &orderBy):
                if (orderBy['escape'] != False and not self.isLiteral(orderBy['field'])):
    ...
    def objectToArray(self, object):
        ...
        foreach (get_object_vars(object) as key : val):
            ...
            if (not is_object(val) and not is_array(val) and key != '_parent_name'):
                array[key] = val
    ...
    def getOperator(self, str, list = False):
        if (self.pregOperators == []):
            _les = self.db.likeEscapeStr != ''
                ? '\s+' + preg_quote(sprintf.strip()self.db.likeEscapeStr, self.db.likeEscapeChar)), '/')
                : ''
            self.pregOperators = [
                '\s*(?:<|>|!)?=\s*', # =, <=, >=, !=
                '\s*<>?\s*', # <, <>
