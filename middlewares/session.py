...
# 	def __init__(self, table='sessions'):
# 		super().__init__()
# 		global tableName
# 		tableName = table
# 		self.connect()
...
# 	def connect(self):
# 		self.dbconfig = get_global('config').get('database.default', raw=True)
# 		self.db = SqliteDatabase(self.dbconfig['path'], self.dbconfig['debug'])
		
# 		class sessions(self.db.Model):
# 			def __init__(self):
# 				self.tableName = tableName
# 				self.column('id', 'integer primary key')
# 				self.column('key', 'text', null=True)
# 				self.column('data', "text")
...
# 			data = json.load(db.models[tableName].get(key=sid, as_json=True))[0]["data"]
# 			return self.session_class(data, sid, False)
			
# 	def list(self):
# 		data = json.load(db.models[tableName].get(as_json=True))
# 		return [self.session_class(data[x]['data'], data[x]['key'], False) for x in data]
	
# 	def delete(self, session):
# 		if self.is_valid_key(session.sid):
# 			db.models[tableName].delete(key=session.sid)
		
# 	def save(self, session):
# 		if self.is_valid_key(session.sid):
# 			db.execute(
# 				db.models[tableName]._query.update(data=json.dumps(session)).where(key=session.sid)
# 			)
