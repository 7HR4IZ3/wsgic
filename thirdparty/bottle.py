    ...
    def add(self, rule, method, target, name=None):
        ...
        for key, mode, conf in self._itertokens(rule):
            if mode:
                ...
                conv = self.filters[mode](conf)
                mask, in_filter, out_filter = conv.regex, conv.to_python, conv.to_url
                if not key:
                    pattern += '(?:%s)' % mask
                    key = 'anon%d' % anons
                    anons += 1
...
class Route(object):
    ...
    def get(self, key, default=None):
        ...
        depr(0, 13, "Route.get() is deprecated.",
    ...
    def POST(self):
        ...
        for item in data:
            if item.filename is None:
                ...
                value = FileUpload(item.file, item.name, item.filename, item.headers)

            if item.name in post:
                if isinstance(post[item.name], list):
                    post[item.name].append(value)
                else:
                    post[item.name] = [post[item.name], value]
            else:post[item.name] = value
        return post
