...
from .events import Event, EventTarget, MouseEvent
from .style import CSSStyleDeclaration as Style, StyleSheetList
from .helpers import *
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        try:
            if name == "args":
                ...
                self._update_parents()
                return
            if isinstance(name, str) and name.startswith("on"):
                self.addEventListener(name.lstrip("on"), value)
        except Exception as e:
            print(e)
            # pass
        super().__setattr__(name, value)
...
class Element(Node):
    ...
    @property
    def innerText(self):
        return "".join([str(each) for each in self.args])
    ...
    def setAttribute(self, attribute, value):
        ...
        try:
            if attribute[0:1] != "_":
                attribute = "_" + attribute
            if isinstance(attribute, str) and attribute.startswith("on"):
                self.addEventListener(attribute.lstrip("on"), value)
