...
class EventTarget:
    ...
    def __init__(self, *args, **kwargs) -> None:
        self.listeners = GLOBAL_LISTENERS[self] = {}
        for key, value in kwargs.items():
            if isinstance(key, str) and key.startswith("on"):
                self.addEventListener(key.lstrip("on"), value)
    ...
    def dispatchEvent(self, event, *args, **kwargs):
        ...
        for thing in stack:
            try:
                thing(event, *args, **kwargs)
                # type(thing, (Event,), self)
            except Exception as e:
                print(e)
                thing(*args, **kwargs)  # try calling without params, user may not create param
